...
export const VIP_LEVEL_THRESHOLDS = [
  { level: 1, unlockBalance: 0, name: "VIP 1", label: "Bronze", dailyProfit: 0.5 },
  { level: 2, unlockBalance: 1500, name: "VIP 2", label: "Silver", dailyProfit: 2 },
  { level: 3, unlockBalance: 5000, name: "VIP 3", label: "Gold", dailyProfit: 6 },
  { level: 4, unlockBalance: 10000, name: "VIP 4", label: "Diamond", dailyProfit: 12 },
];
...
export async function approveVipRequest(requestId, { adminUid = "", note = "" } = {}) {
  ...
  await db.collection("users").updateOne(
    { uid: req.uid },
    { $set: { vipLevel: req.toLevel, vipNotifiedLevel: Math.max(req.toLevel, Number(req.fromLevel || 1)), updatedAt: new Date() } }
  );
...
export async function setUserVipLevel(uid, level, { adminUid = "", action = "set", note = "" } = {}) {
  ...
  await db.collection("users").updateOne(
    { uid },
    { $set: { vipLevel: target, vipNotifiedLevel: target, updatedAt: new Date() } }
  );
