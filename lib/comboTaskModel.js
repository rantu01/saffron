...
export async function updateComboConfig(updates) {
  ...
  const allowed = ["enabled", "probability", "minOrders", "maxOrders", "commissionPercent", "progressionLevels", "positions", "demoPositions", "mainPositions", "userPositionOverrides", "userComboSettings", "firstComboMin", "firstComboMax", "secondComboMin", "secondComboMax", "profitThreshold", "daysUntilFirstCombo"];
  const setFields = {};
...
function buildOrdersFromSettings(settings, config, stage) {
...
export async function applyUserComboConfig(uid) {
  ...
  const userDoc = await db.collection("users").findOne({ uid }, { projection: { comboStage: 1 } });
  const stage = Number(userDoc?.comboStage || 0) >= 2 ? 2 : 1;

  const orders = buildOrdersFromSettings(settings, config, stage);
  const commissionPercent =
