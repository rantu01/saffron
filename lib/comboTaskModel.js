...
import { getVipTier } from "./vipModel";
...
export function computeTaskProfit(amount, vipLevel = 1) {
  const tier = getVipTier(vipLevel);
  return roundCurrency(Number(amount || 0) * (Number(tier.profitIncrease || 0.5) / 100));
}

// Normal (single) task values scale to a small fraction of the account balance.
// With a $40 balance this yields values between ~$7 and ~$12.
export function generateNormalTaskAmount(userBalance) {
  return roundCurrency(userBalance * (0.175 + Math.random() * 0.125));
}
...
export async function saveUserComboSettings(uid, settings) {
  ...
  const clean = {
    ...
    position: settings.position ? Math.max(1, Math.min(100, Number(settings.position))) : null,
...
// Combined (combo) order values are always GREATER than the account balance.
// When `userBalance` is provided, the generated amount is guaranteed to exceed
// it (using a balance-scaled margin). Otherwise the configured fixed range is
// used as a fallback.
export function generateStageOrderAmount(stage, config, userBalance = 0) {
  const balance = Number(userBalance || 0);

  if (balance > 0) {
    const margin = 1.2 + Math.random() * 0.6; // 20%-80% above current balance
    return roundCurrency(balance * margin);
  }
...
export function getComboPosition(config, setNumber, uid, accountType = "main") {
  ...
  const resolve = (raw) => {
    const p = Math.max(1, Math.min(100, Number(raw) || 1));
...
export async function createStageComboTask(uid, setNumber, config, stage) {
  ...
  if (userSettings && Array.isArray(userSettings.orderAmounts) && userSettings.orderAmounts.length) {
    ...
    orders = Array.from({ length: userSettings.numOrders }).map((_, i) => ({
      orderNumber: i + 1,
      requiredAmount: generateStageOrderAmount(stage, config, userBalance),
      status: "pending",
      completedAt: null,
    }));
  } else {
...
async function restoreNormalTaskFromTemplate(db, taskDoc) {
  ...
  const user = await db.collection("users").findOne({ uid: taskDoc.assigneeUid }, { projection: { availableBalance: 1, vipLevel: 1 } });
  const amount = generateNormalTaskAmount(Number(user?.availableBalance || 0));
  const profit = computeTaskProfit(amount, Number(user?.vipLevel || 1));
